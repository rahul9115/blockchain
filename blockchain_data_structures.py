...
open_transactions=[]
sender="Max"
block={"previous_hash":' ',"transactions":[],"proof":100}
...
def load_data():
    with open("blockchain.p","rb") as f:
        ...
        balance_miner=int(file_content['balance_miner'])
...
def mine_block(balance_miner):
    ...
    mine={'miner_name':sender,'reward':balance_miner}
...
while True:
    ...
    if a==2:
        ...
        proof=proof_of_work()
        print(proof)
        block={"previous_hash":hashed,"transactions":open_transactions,"proof":proof}
        blockchain.append(block)
        save_data()
        open_transactions=[]
    ...
    if a==3:
        ...
        for i in range(len(blockchain)-1,0,-1):
            print("in loop")
            print(hashing(blockchain[i-1]),blockchain[i]['previous_hash'])
